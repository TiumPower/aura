#!/usr/bin/env bash
#
# Nightly backup of the Aura production database and uploaded files.
#
#   Runs from cron as the deploy user:
#     15 3 * * * /var/www/aura/shared/bin/aura_backup.sh >> /var/www/aura/shared/log/backup.log 2>&1
#
#   Keeps BACKUP_KEEP_DAYS (default 30) of daily snapshots in /var/www/aura/backups,
#   mode 700 — the dumps contain every tenant's contracts, bills and ID documents.
#
# RESTORE:
#   dropdb  -h localhost -U aura aura_production
#   createdb -h localhost -U aura aura_production
#   pg_restore -h localhost -U aura -d aura_production --no-owner db/aura_YYYYmmdd.dump
#   tar xzf storage/storage_YYYYmmdd.tar.gz -C /var/www/aura/shared
#
...
set -euo pipefail
# Dumps contain every tenant's contracts and ID documents — owner-only.
umask 077

APP_ROOT="${APP_ROOT:-/var/www/aura}"
...
DB_PASS="$(env_get AURA_DATABASE_PASSWORD)"
[ -n "$DB_PASS" ] || fail "missing AURA_DATABASE_PASSWORD in $ENV_FILE"
DB_NAME="$(env_get AURA_DATABASE_NAME)"; DB_NAME="${DB_NAME:-aura_production}"
DB_USER="$(env_get AURA_DATABASE_USER)"; DB_USER="${DB_USER:-aura}"
DB_HOST="$(env_get AURA_DATABASE_HOST)"; DB_HOST="${DB_HOST:-localhost}"

mkdir -p "$BACKUP_ROOT/db" "$BACKUP_ROOT/storage"
chmod 700 "$BACKUP_ROOT"

# ---- Database ---------------------------------------------------------------
DUMP="$BACKUP_ROOT/db/aura_${STAMP}.dump"
