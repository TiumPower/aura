...
module PushSender
  ...
  # Deliver to a landlord/staff user's devices (cross-workspace: a user may own
  # several workspaces, so ignore the tenant scope when finding their devices).
  def deliver_to_users(user_ids, title:, body:, path: "/", icon: nil)
    return unless configured? && user_ids.present?
    payload = JSON.generate(title: title, body: body.to_s, path: path, icon: icon)
    ActsAsTenant.without_tenant do
      PushSubscription.where(user_id: user_ids).find_each { |sub| send_one(sub, payload) }
    end
  end
