...
class BrevoMailDelivery
  ...
  def deliver!(mail)
    ...
    from_email = addr&.address.presence || Array(mail.from).first || ENV.fetch("MAIL_FROM", "no-reply@aura.tiumpower.com")
    from_name  = addr&.display_name.presence || ENV.fetch("MAIL_FROM_NAME", "Aura")
    body = {
